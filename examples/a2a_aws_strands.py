"""A2A Peptides — Strands Agents (AWS) stock A2A client provider against a country agent. No adapter.
pip install strands-agents strands-agents-tools cryptography

Discovers the card, verifies its ES256 signature against the hub keyring (kid a2ap-2026-09), sends one structured read,
...
async def main():
    ...
    read = artifact_data(await provider._send_message(message_text=json.dumps({"skill": "get_pathway_status", "params": {"substance": "semaglutide", "pathway": "licensed_medicine"}}), target_agent_url=AGENT))
