"""A2A Peptides — stock a2a-sdk (Google) client against a country agent. No adapter.
pip install a2a-sdk httpx cryptography

Fetches the card, verifies its ES256 signature against the hub keyring (kid a2ap-2026-09), sends one structured read
as a DataPart {skill, params}, then a transacting skill without a bearer to show the typed refusal (DENY_UNLICENSED_AGENT).
Pick any country agent: the hostname is the country code in lower case before .a2a-peptides.ai (the United Kingdom is `uk`).
"""
...
AGENT = sys.argv[1] if len(sys.argv) > 1 else "https://uk.a2a-peptides.ai"
KEYRING = "https://a2a-peptides.ai/.well-known/jwks.json"
UA = {"User-Agent": "a2a-peptides-kit-example/1.0"}  # the edge refuses requests with no User-Agent
...
async def main():
    async with httpx.AsyncClient(timeout=120) as http:
        ...
        read = artifact_data(await client.send_message(SendMessageRequest(id="1", params=MessageSendParams(message=message("get_pathway_status", {"substance": "semaglutide", "pathway": "licensed_medicine"}, "m1")))))
