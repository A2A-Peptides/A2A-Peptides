"""A2A Peptides — reference MCP client (streamable-HTTP). Lists the tools, then runs the gate order.
pip install mcp
"""
import asyncio
...
async def main():
    async with streamablehttp_client(URL) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            g = await session.call_tool("gate_transaction", {"substance": "semaglutide", "pathway": "licensed_medicine", "actor_class": "wholesaler", "channel": "B2B", "ship_to": "US-CA"})
