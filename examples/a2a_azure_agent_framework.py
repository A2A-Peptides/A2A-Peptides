"""A2A Peptides — Microsoft Agent Framework (Azure) stock A2A agent client against a country agent. No adapter.
pip install agent-framework-a2a a2a-sdk httpx cryptography

Resolves the card, verifies its ES256 signature against the hub keyring (kid a2ap-2026-09), sends one structured read,
...
async def main():
    async with httpx.AsyncClient(timeout=120) as http:
        ...
        agent = A2AAgent(name="peptides", agent_card=card, url=AGENT + "/a2a", http_client=http)
        read = artifact_data(await agent.run(json.dumps({"skill": "get_pathway_status", "params": {"substance": "semaglutide", "pathway": "licensed_medicine"}})))
